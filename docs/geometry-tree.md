Title: Geometry tree - IfcOpenShell 0.8.0 documentation
...
![Image 6: ../_images/geometry-tree.png](https://docs.ifcopenshell.org/_images/geometry-tree.png)The most efficient way to build a tree is by using the iterator. If the native OpenCASCADE shape is added to the tree, a **UB Tree** is built. Alternatively, if triangulation is added to the tree, a **BVH Tree** is built. The type of tree determines the type of operation you can perform.
...
tree.add_element(iterator.get())
        ...
        \# tree.add\_element(iterator.get\_native())

        if not iterator.next():
            break

## Clashing or selecting geometry from a geometry tree[¶](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#clashing-or-selecting-geometry-from-a-geometry-tree "Link to this heading")
...
- [Detecting intersection clashes between elements](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#detecting-intersection-clashes-between-elements) detects when an element intersects with another element. This is the most common type of clash detection used when coordinating designs. For example, you might want to know if any pipes go through structural columns or beams.
- [Detecting collision clashes between elements](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#detecting-collision-clashes-between-elements) detects when an element touches another element. It is the fastest type of clash detection but does not consider the distance that an element goes inside another element. This considers surfaces only so it works on non-manifold geometry but will not detect if an element is completely within another element.
- [Detecting clearance clashes between elements](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#detecting-clearance-clashes-between-elements) detects when an element comes near to another element within a clearance threshold. This is the slowest type of clash detection. It works on non-manifold geometry and does not consider inside vs outside. Elements like pipe and ducts with insulation, structural openings, and equipment will typically require clearance checks.
...
- [Selecting elements using bounding boxes](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#selecting-elements-using-bounding-boxes) lets you query for elements that contain a point or another element. However, it only checks the bounding box of elements instead of their exact geometry. This is the fastest approach and is recommended if you don’t need precise geometry selection.
- [Selecting elements using precise geometry](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#selecting-elements-using-precise-geometry) lets you query for elements that contain a point, a sphere, or another element. This is similar to selecting using bounding boxes, but additionally considers the actual geometry of the element. This is slower but more precise.
- [Selecting elements using a ray](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#selecting-elements-using-a-ray) lets you query for elements that intersect with a ray.

## Detecting intersection clashes between elements[¶](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#detecting-intersection-clashes-between-elements "Link to this heading")
...
b_name \= element2.get_argument(2)
    ...
    clash\_type \= \["protrusion", "pierce", "collision", "clearance"\]\[clash.clash\_type\],
...
1.  When an element X protrudes inside element Y, where element Y is manifold. In this case, a protrusion distance is calculated as the deepest point of element X to the closest surface of element Y. `P1` is defined as the XYZ coordinate on element X, and `P2` is defined as the nearest point on the surface of element Y.
2.  When an element X pierces element Y, such that an edge of element X enters element Y and leaves through another face. In this case, a piercing distance is calculated as the distance where that edge is inside element Y. `P1` is defined as the point on an edge of element X which enters element Y, and `P2` is the point where that edge leaves element Y.
3.  When neither X or Y is manifold, we cannot detect protrusion or piercing, so instead when X and Y have any touching face. This is the same as the `clash_collision_many` check below. The distance is considered to be zero and ignores your specified tolerance. `P1` and `P2` are equal and represent an arbitrary XYZ point where the two elements touch.

## Detecting collision clashes between elements[¶](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#detecting-collision-clashes-between-elements "Link to this heading")
...
## Detecting clearance clashes between elements[¶](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#detecting-clearance-clashes-between-elements "Link to this heading")
...
## Selecting elements using bounding boxes[¶](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#selecting-elements-using-bounding-boxes "Link to this heading")
...
## Selecting elements using precise geometry[¶](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#selecting-elements-using-precise-geometry "Link to this heading")
...
## Selecting elements using a ray[¶](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_tree.html#selecting-elements-using-a-ray "Link to this heading")
