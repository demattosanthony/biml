Title: Geometry processing - IfcOpenShell 0.8.0 documentation
...
## Individual processing[¶](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_processing.html#individual-processing "Link to this heading")
...
This section describes individual processing only. This is useful for learning how geometry processing works, but is not recommended for practical applications. See the [Geometry iterator](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_processing.html#geometry-iterator) section below after reading this to see how to process geometry with multiple threads.
...
print(style.original_name())
    ...
    if style.has\_diffuse:
        print(style.diffuse)

    \# Each style may have transparency data
    if style.has\_transparency:
...
## Geometry iterator[¶](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_processing.html#geometry-iterator "Link to this heading")

IfcOpenShell provides a geometry iterator function to efficiently process geometry in an IFC model. The iterator is always used in IfcConvert, and may also be invoked in C++ or in Python. It offers the same features as the `create_shape()` function for [Individual processing](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_processing.html#individual-processing).
...
There are a variety of configuration settings to get different output. For example, you may filter elements from processing, extract 2D data, or return non-triangulated OpenCASCADE BReps. For more information on the various settings, see [Geometry Settings](https://docs.ifcopenshell.org/ifcopenshell/geometry_settings.html).
...
## Manual parsing[¶](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_processing.html#manual-parsing "Link to this heading")

IfcOpenShell lets you traverse any IFC entity graph. This means it is possible for you to manually browse through the `Representation` attribute of IFC elements, and parse the corresponding IFC shape representations yourself instead of using generic geometric processing such as [Individual processing](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_processing.html#individual-processing) and the [Geometry iterator](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_processing.html#geometry-iterator).
...
print(circle.Radius)
    ...
    \# In SI meters
    print(circle.Radius \* unit\_scale)

Given the advanced nature of manual processing, it is generally not recommended except in specific tasks.

## Geometry serialisation[¶](https://docs.ifcopenshell.org/ifcopenshell-python/geometry_processing.html#geometry-serialisation "Link to this heading")

Geometry may be serialised into many different formats using [IfcConvert](https://docs.ifcopenshell.org/ifcconvert.html). Alternatively, you may also access the serialiser with Python to customise the conversion, such as by writing a script that modifies the IFC on the fly before converting it, or writing complex include and exclude filters.
